...
    if (emails.length === 0) {
      console.log("No se encontraron correos nuevos de Idealista (o los encontrados son respuestas/notificaciones).");
      return;
    }
...
        // Verificamos si ya existe un Lead con este email y vinculado a esta propiedad.
        const existingLeadId = await checkIfLeadExists(parsedData.email_cliente, parsedData.referencia);

        if (existingLeadId) {
          console.log(`AVISO: Lead ya existe y está vinculado a esta propiedad (${existingLeadId}). Saltando la creación del registro.`);
          
          // Aún podemos enviar el correo al cliente si es el primer mensaje.
          if (parsedData.email_cliente && parsedData.email_cliente !== "-") {
            await sendClientEmail(accessToken, parsedData);
          }
          
        } else {
          // Si NO existe, proceder a crear y vincular.
...
          }
          
          // Enviar correo comercial al asignado
...
  // === FILTRO MEJORADO: Excluir respuestas y notificaciones ===
  const filter = `isRead eq false and from/emailAddress/address eq '${SENDER_FILTER}' and not startswith(subject, 'Respuesta de') and not startswith(subject, 'Tu favorito tiene')`;
  
  const url = `https://graph.microsoft.com/v1.0/users/${OUTLOOK_USER_EMAIL}/messages?$filter=${encodeURIComponent(filter)}&$select=id,subject,body,from`;
  
...
  const emailContent = {
    message: {
      subject: `Gracias por tu interés en la propiedad ${data.direccion_inmueble} - Iceberg Solutions`,
      body: {
        contentType: "Html",
        content: `
...
          <br><br>
          Queremos informarte que en **Iceberg Solutions** somos los propietarios de la vivienda que has visto en Idealista.
          <br><br>
...
// FUNCIÓN AGREGADA PARA PREVENIR DUPLICADOS
async function checkIfLeadExists(email, referencia) {
  // Si no hay email o referencia, no podemos buscar el duplicado de forma fiable.
  if (email === "-" || !email || referencia === "-" || !referencia) {
    return null;
  }

  // Filtro: Buscar leads con el mismo email Y vinculados (o que deban vincularse) a la misma propiedad.
  const filterFormula = `AND({Email} = '${email}', {id test } = '${referencia}')`;
  const encodedFilter = encodeURIComponent(filterFormula);

  const url = `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${AIRTABLE_TABLE_ID}?filterByFormula=${encodedFilter}&maxRecords=1&fields[]=Email`;
  
  console.log(`DEBUG: Buscando duplicados con formula: ${filterFormula}`);
  
  try {
...
  } catch (error) {
    console.error("Error al verificar duplicado en Airtable:", error.message);
    return null;
  }
}

async function createAirtableRecord(data) {
  // === Limpiar el teléfono para evitar espacios, etc. ===
...
        "Status": "No contactado" // Añadido el estado inicial para la consistencia del flujo.
      }
    }]
  };
